document.addEventListener('DOMContentLoaded', function() {
    ...
    ideaSuggestions.forEach(suggestion => {
        suggestion.addEventListener('click', function() {
            ...
            window.location.href = `index.html?prompt=${encodeURIComponent(this.textContent)}`;
        });
    });
    ...
    function shuffleVideos() {
        ...
        for (let i = videoCards.length - 1; i > 0; i--) {
            ...
            [videoCards[i].innerHTML, videoCards[j].innerHTML] = [videoCards[j].innerHTML, videoCards[i].innerHTML];
        }
    ...
    function checkURLParams() {
        ...
        if (filterParam) {
            ...
            filterOptions.forEach(option => {
                if (option.previousElementSibling.textContent.trim().includes(filterParam)) {
